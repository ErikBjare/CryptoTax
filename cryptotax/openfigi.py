...
def get_by_isin(isin, exchcode="US"):
    ...
    r = requests.post("https://api.openfigi.com/v1/mapping", json.dumps(data), headers=headers)
    if b"Too Many Requests" in r.content:
        logger.error("Ratelimited by OpenFIGI, put an API key in the 'OPENFIGI_APIKEY' environment variable")
        return None
